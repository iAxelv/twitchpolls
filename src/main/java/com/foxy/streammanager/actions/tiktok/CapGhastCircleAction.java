...
public class CapGhastCircleAction implements ActionStrategy {
    @Override public void execute(ActionContext context) {
        int amount = Math.max(1, context.config().getInt("amount", 10));
        double radius = Math.max(0.5, context.config().getDouble("radius", 10.0));
        ...
        for (int index = 0; index < amount; index++) {
            ...
            Ghast ghast = (Ghast) context.world().spawnEntity(location, EntityType.GHAST);
            ghast.customName(Component.text(donor));
            ghast.setCustomNameVisible(true);
            ghast.setTarget(context.player());
        }
    }
}