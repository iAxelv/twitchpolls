...
public class HappyHolidaysAction implements ActionStrategy {
    ...
    public void execute(ActionContext context) {
        ...
        if (!amounts.isEmpty()) {
            ...
            int minimum = Math.max(1, context.config().getInt("min-apples", 1));
            int maximum = Math.max(minimum, context.config().getInt("max-apples", 2));
